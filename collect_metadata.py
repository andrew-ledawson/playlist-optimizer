...
for candidate_playlist in ytm_playlists:
    ...
    if local_playlist_author_name == user_name or local_playlist_author_id == user_name:
        ...
        remote_playlist_contents = run_API_request(lambda : YTM.get_playlist(playlistId=local_playlist.yt_id, limit=int(candidate_playlist['count']) + 1), "to get the songs from YouTube Music playlist \"" + local_playlist.name + "\"")
        ...
        for song_count, playlist_song in enumerate(remote_playlist_contents['tracks']):
            ...
            if song_count % 10 == 9:
                print("Checking song " + str(song_count + 1) + " of " + str(len(remote_playlist_contents['tracks'])) + "... ")
        ...
        playlist_file.close()
        print("Saved playlist \"" + local_playlist.name + "\". ")
